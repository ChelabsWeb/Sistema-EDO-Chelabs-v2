...
function translateError(errorMessage: string): string {
  const errorTranslations: Record<string, string> = {
    'User already registered': 'El email ya se encuentra registrado',
    'Password should be at least 6 characters': 'La contraseña debe tener al menos 6 caracteres',
    'Invalid email': 'El formato del email es inválido',
    'Network error': 'Error de conexión. Verificá tu internet.',
  }
  return errorTranslations[errorMessage] || 'Error al registrarse. Intentá de nuevo.'
}
...
export default function RegisterPage() {
  ...
  const handleRegister = async (e: React.FormEvent) => {
    ...
    // Opcional: Redirigir al inicio de sesión luego de unos segundos
    setTimeout(() => {
    ...
    <div className="min-h-screen flex items-center justify-center bg-background p-4 selection:bg-primary/30">
      <div className="w-full max-w-md">
        ...
        <Card>
          ...
          <CardContent>
            {success ? (
              <div className="p-4 bg-emerald-50 dark:bg-emerald-500/10 border border-emerald-200 dark:border-emerald-500/20 rounded-xl text-center space-y-2">
                <h3 className="text-emerald-600 dark:text-emerald-400 font-bold">¡Registro Exitoso!</h3>
                <p className="text-sm text-emerald-600/80 dark:text-emerald-400/80">
                  Por favor revisá tu casilla de correo electrónico para confirmar tu cuenta antes de iniciar sesión.
                </p>
              </div>
            ) : (
        ...
        <p className="text-center text-xs text-muted-foreground mt-8">
          ¿Ya tienes una cuenta? <Link href="/auth/login" className="font-semibold hover:text-primary transition-colors">Iniciar Sesión</Link>
        </p>
